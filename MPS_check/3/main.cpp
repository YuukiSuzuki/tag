...
int main(){
	...
	cv::projectPoints(AsrcPoints3D,rotation,translation,intrinsic,distortion,BdstPoints2D);
       ...
       for(int i=0; i<14 ; i++){
			...
			cv::Mat matB(2,1,CV_64F);//基準のMPSの平行行列
			int x = id_nunber[i].id/100;//MPSのカラー
			int y = (id_nunber[i].id/10)%10;//MPSの種類
			matB.at<double>(0,0) =  -RefboxMPS[x][y].x;
			matB.at<double>(1,0) =  -RefboxMPS[x][y].y;
			double a = RefboxMPS[x][y].a;

			//std::cout << "matB" << matB << std::endl;
			
			cv::Mat matC(2,2,CV_64F);//基準のMPSの回転行列
			if(id_nunber[i].id%2 == 0){
				a = a + (2 * pi);
			}
			matC.at<double>(0,0) = std::cos(a);
			matC.at<double>(1,0) = std::sin(a);
			matC.at<double>(0,1) = -(std::sin(a));
			matC.at<double>(1,1) = std::cos(a);
			
			//std::cout << "MatC" <<MatC << std::endl;
			
			for(int k=0; k<14; k++){
				if(id_nunber[k].id!=333){
					cv::Mat matX(4,1,CV_64F,id_nunber[k].b);//カメラ座標系でのMPSの位置
					int ref_x = id_nunber[k].id/100;//id_nunber[k].idの同じ色の取得
                        		int ref_y = (id_nunber[k].id/10)%10;//id_nunber[k].idの同じMPSの種類の取得
					cv::Mat refm(2,1,CV_64F);//id_nunber[k].idのレフボックスでの位置
					cv::Mat refmF(2,1,CV_64F);//レフボックスの平行移動したのち回転移動
					cv::Mat refmE(2,1,CV_64F);//レフボックスの平行移動
					cv::Mat matF(4,1,CV_64F);//基準のMPSを平行移動したもの
					cv::Mat matFF(4,1,CV_64F);//基準のMPSのタグの位置から中心に移動
					...
					double ad[4]={0.0,-4.0,17.5,1.0};//output
					if(id_nunber[i].id%2 == 0){
						matFF.at<double>(0,0) = matF.at<double>(0,0);
						matFF.at<double>(1,0) = matF.at<double>(1,0)+ac[1];
						matFF.at<double>(2,0) = matF.at<double>(2,0)+ac[2];
						matFF.at<double>(3,0) = matF.at<double>(3,0);
					}else{
						matFF.at<double>(0,0) = matF.at<double>(0,0);
						matFF.at<double>(1,0) = matF.at<double>(1,0)+ad[1];
						matFF.at<double>(2,0) = matF.at<double>(2,0)+ad[2];
						matFF.at<double>(3,0) = matF.at<double>(3,0);

					}
					refm.at<double>(0,0) = RefboxMPS[ref_x][ref_y].x;
					refm.at<double>(1,0) = RefboxMPS[ref_x][ref_y].y;

					//std::cout << "refm" << refm << std::endl;

					refmE.at<double>(0,0) = refm.at<double>(0,0) + matB.at<double>(0,0);
                                        refmE.at<double>(1,0) = refm.at<double>(1,0) + matB.at<double>(1,0);
					...
					std::cout << "MPS=" << matFF << std::endl;
                                        
					//std::cout << "refmF=" << refmF << std::endl;

					for(int j = 0; j < 8 ; j++){
						double ro = j*pi/4;//カメラから見たMPSを回転させる
						cv::Mat roMPS(2,1,CV_64F);
						roMPS.at<double>(0,0) = matFF.at<double>(1,0) * std::cos(ro) - matFF.at<double>(2,0) * std::sin(ro);
						roMPS.at<double>(1,0) = matFF.at<double>(1,0) * std::sin(ro) + matFF.at<double>(2,0) * std::cos(ro);

						//std::cout << "roMPS" << roMPS << std::endl;

						double d = sqrt(pow(refmE.at<double>(0,0) - roMPS.at<double>(0,0),2) + (pow(refmE.at<double>(1,0) - roMPS.at<double>(1,0),2)));

						//std::cout << "distance   =   " << d << std::endl;

						if(0 <= d && d <= 100){
						       std::cout << "基準MPS" << id_nunber[i].id << "は角度(ラジアン)" << ro << "のとき比較したMPS" << id_nunber[k].id << "は一致したよー" << std::endl;	
						}
