...
int main(int argc, char *argv[]){
	...
	cv::FileStorage fs2("/home/robotino/catkin_ws/src/tag/camera.xml",cv::FileStorage::READ);
	if(!fs2.isOpened()){
	...
	ros::init(argc , argv, "robo2_camera");
        ros::NodeHandle nh;
        ros::Publisher Robo_pub = nh.advertise<std_msgs::Float64MultiArray>("ca_2",1000);
...
//while(cap.read(frame) && ros::ok()){
		   ...
		   //タグ原点座標系でのカメラ原点の座標をロボットの中心座標系でのカメラ原点の座標に変換
		     ...
		     //     ____       ____         ____
			     ...
			     id_nunber[i].c[2] = -matY.at<double>(1,0);
                     ...
                     if(id_nunber[i].id==17 || id_nunber[i].id==77 || id_nunber[i].id==81){
			     id_nunber[i].c[0] = matY.at<double>(0,0)-67.5;
			     id_nunber[i].c[1] = matY.at<double>(2,0)-82.0;
			     id_nunber[i].c[2] = -matY.at<double>(1,0);

                     }
                     if(id_nunber[i].id==18 || id_nunber[i].id==78 || id_nunber[i].id==82){ 
	...
	for(int i=0; i<10 ; i++){
		if(id_nunber[i].id!=0){
			...
			//robotino2
		        if(id_nunber[i].id==33 || id_nunber[i].id==34 || id_nunber[i].id==77 || id_nunber[i].id==78){
